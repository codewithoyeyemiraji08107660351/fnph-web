...
import type { CentreRow, DoctorAvailability, Page, RoomOption } from '../types'
...
export const schedulingApi = {
  ...
  list: (from: string, to: string) => api.get<Publication[]>('/admin/schedules', { params: { from, to } }),
  slots: (id: string) => api.get<SlotRow[]>(`/admin/schedules/${seg(id)}/slots`),
...
export const centresAdminApi = {
  ...
  capabilities: (id: string) => api.get<CapabilityState[]>(`/admin/centres/${seg(id)}/capabilities`),
...
export const notificationsAdminApi = {
  templates: () => api.get<TemplateRow[]>('/admin/notifications/templates'),
  ...
  broadcasts: () => api.get<BroadcastRow[]>('/admin/notifications/broadcasts'),
}
...
export const ictApi = {
  imports: () => api.get<EhrImport[]>('/admin/ehr-imports'),
  ...
  verificationQueue: (status?: VerificationStatus, page = 0) =>
    api.get<Page<VerificationRequest>>('/admin/verification-requests', { params: { status, page, size: 50 } }),
  ...
  quarantined: () => api.get<Array<{ publicId: string; originalFileName: string; uploadedBy: string; uploadedAt: string; scanResult?: string }>>('/admin/uploads/quarantined'),
}
