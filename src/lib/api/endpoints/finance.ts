...
export const financeApi = {
  ...
  wallets: () => api.get<WalletRow[]>('/finance/wallets'),
  alerts: () => api.get<WalletAlert[]>('/finance/wallets/alerts'),
  ledger: (centrePublicId: string, page = 0) => api.get<LedgerRow[]>(`/finance/wallets/${seg(centrePublicId)}/ledger`, { params: { page, size: 50 } }),
  ...
  payments: (p: { from?: string; to?: string; status?: string; page?: number }) =>
    api.get<FinancePayment[]>('/finance/payments', { params: { ...p, size: 50 } }),
  ...
  exceptions: (page = 0) => api.get<PaymentException[]>('/finance/exceptions', { params: { page, size: 50 } }),
