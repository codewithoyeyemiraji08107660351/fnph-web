...
export function PortalHome() {
  ...
  if(consent.isLoading || triage.isLoading) return <p>Checking your first-time setup…</p>
  if(consent.isError || triage.isError) return <ErrorState error={consent.error ?? triage.error} onRetry={()=>{void consent.refetch();void triage.refetch()}} />
  if(!consent.data?.publicId || !triage.data?.length) return <Navigate to="/portal/onboarding" replace />
