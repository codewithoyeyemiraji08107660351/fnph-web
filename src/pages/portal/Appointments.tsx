...
function History({ id }: { id: string }) {
  ...
  if (history.isError)
    return <p className="text-sm text-alarm">History could not be loaded.</p>

  return (
    <ol className="space-y-2 border-l-2 border-line pl-4">
      {history.data?.map((h, i) => (
        <li key={i} className="text-sm">
          <span className="font-bold">{humanise(h.toStatus)}</span>{' '}
          <span className="text-muted">{formatDateTime(h.changedAt)}</span>
...
const watDay = (offset: number) =>
  new Intl.DateTimeFormat('en-CA', { timeZone: DISPLAY_TIME_ZONE }).format(
...
function MoveDialog({
  ...
  const times = useQuery({
    queryKey: ['booking-times', date],
    queryFn: () => bookingApi.times(date),
  })
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={busy || !slot || reason.trim().length < 5}
            onClick={async () => {
              setBusy(true)
              setError(null)
              try {
                await lifecycleApi.reschedule(a.publicId, slot!, reason.trim())
                onDone()
              } catch (err) {
          ...
          >
            {busy ? <Spinner label="Moving" inverted /> : 'Move my appointment'}
          </button>
        </>
      }
    >
      ...
      {times.data?.length === 0 && (
        <p className="mt-3 text-sm text-muted">No times left on this day.</p>
      )}
      <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
        {times.data?.map((t) => (
          <button
            key={t.slotPublicId}
            type="button"
            aria-pressed={slot === t.slotPublicId}
            onClick={() => setSlot(t.slotPublicId)}
            className={`rounded-[12px] border px-2 py-2 font-display font-extrabold ${
              slot === t.slotPublicId
      ...
      <TextField
        ...
        placeholder="I have to travel that day"
      />
    </Dialog>
  )
}
...
function Card({ a, highlighted }: { a: Appointment; highlighted: boolean }) {
  ...
  const changeable =
    (a.status === 'APPROVED' || a.status === 'AWAITING_APPROVAL') &&
    (parseServerTime(a.appointmentDate)?.getTime() ?? 0) > now
  ...
  const canJoin =
    JOINABLE.has(a.status) && opensAt !== undefined && now >= opensAt && now < endsAt
  const held =
    a.status === 'SLOT_HELD' && (parseServerTime(a.heldUntil)?.getTime() ?? 0) > now
    ...
    <li className={`card p-5 ${highlighted ? 'ring-2 ring-accent' : ''}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-display text-lg font-extrabold">
            {formatDateTime(a.appointmentDate)} WAT
          </p>
      ...
      {a.status === 'REJECTED' && a.rejectedReason && (
        <Alert
          tone="warning"
          className="mt-4"
          title="The hospital could not take this appointment"
        >
      ...
      {a.status === 'AWAITING_APPROVAL' && (
        <p className="mt-3 text-sm text-muted">
          Paid. The Hub Coordinator is assigning your doctor and room.
        </p>
      )}
      {JOINABLE.has(a.status) && !canJoin && opensAt && now < opensAt && (
        <p className="mt-3 text-sm text-muted">
          The room opens at {formatTime(a.joinWindowOpensAt)} WAT. Find somewhere private
          before then.
        </p>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {changeable && (
          ...
          >
            <i aria-hidden className="bi bi-paperclip" /> Attach a result
          </button>
        )}
        <button
          type="button"
      ...
      {action === 'move' && (
        <MoveDialog
          ...
          onDone={() => {
            setAction(null)
            toast('Moved. The hospital will confirm the new time.')
            refresh()
          }}
        />
      )}
      {action === 'attach' && (
        <UploadDialog
          title="Attach a result for this appointment"
          referenceId={a.publicId}
          categories={['LABORATORY_RESULT', 'VITALS_EVIDENCE', 'SUPPORTING_DOCUMENT']}
          onClose={() => setAction(null)}
          onDone={() => {
            setAction(null)
            toast('Attached. The care team can see it.')
          }}
        />
      )}
      {action === 'cancel' && (
        <ReasonDialog
          ...
          onConfirm={async (reason) => {
            const r = await lifecycleApi.requestChange(a.publicId, {
              requestType: 'CANCEL',
              reason,
            })
            toast(
              r.withinNoticePeriod
                ? 'Request sent. You will be told the decision.'
                : `Request sent. This is less than ${r.requiredNoticeHours} hours before the appointment, so it may not be accepted.`,
              r.withinNoticePeriod ? 'success' : 'info',
            )
            setAction(null)
          }}
        />
...
export function Appointments() {
  ...
  const rows = [...(mine.data ?? [])].sort(
    (x, y) =>
      (parseServerTime(y.appointmentDate)?.getTime() ?? 0) -
      (parseServerTime(x.appointmentDate)?.getTime() ?? 0),
  )
      ...
      {mine.data?.length === 0 && (
        <div className="card">
          <EmptyState icon="bi-calendar2" title="No appointments yet">
            Book a follow-up consultation when your clinical team has approved remote
            care.
          </EmptyState>
        </div>
      )}
