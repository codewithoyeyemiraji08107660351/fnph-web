...
export function Enrol() {
  ...
  if (step === 'lookup') return <><div className="stage-header"><div><span>Welcome</span><strong>Begin with your FNPH record</strong></div><span className="step-pill">Patient journey</span></div><div className="landing-hero"><div><span className="kicker">FNPH KADUNA TELEPSYCHIATRY</span><h1>Follow-up care, from a calm place of your choosing.</h1><p>Your health is our concern. Use your existing hospital EHR number to enrol, complete the safety and privacy checks, then request a secure consultation.</p><div className="eligibility-badges"><span>✓ Existing patients only</span><span>✓ Previously assessed in person</span><span>✓ Non-emergency follow-up</span></div></div><div className="hero-symbol" aria-hidden><img src="/fnph-logo.png" alt="" /><span>A place for care, healing and renewed hope.</span></div></div><div className="landing-grid"><form className="card lookup-card" onSubmit={lookup} noValidate><span className="eyebrow">Step one · hospital record</span><h2>Enter the EHR number on your card</h2><p>We first match the number to a verified existing FNPH record. You cannot create a new hospital record through this service.</p><label htmlFor="ehr-lookup">Existing FNPH EHR number</label><div className="inline-form"><input id="ehr-lookup" inputMode="numeric" autoComplete="off" maxLength={50} placeholder="For demo, use 204815" value={ehrNumber} onChange={e => setEhrNumber(e.target.value)} required /><button className="button primary" type="submit" disabled={busy}>{busy ? 'Checking…' : 'Find my record →'}</button></div><div className="form-foot"><button className="text-button" type="button" onClick={() => setEhrNumber('204815')}>Use sample EHR 204815</button><button className="text-button" type="button" onClick={() => setStep('help')}>My card number is not recognised</button></div>{error && <p className="form-error" role="alert">{error}</p>}</form><div className="card guide-card"><div className="guide-visual"><span className="play-disc">▶</span><small>Introduction video space</small></div><h2>See how telepsychiatry works</h2><p>Learn about enrolment, booking, the private consultation room and follow-up care.</p><a className="button outline" href="#patient-main">View patient journey</a><small>The approved hospital demonstration video, captions and transcript will be added before launch.</small></div></div></>
